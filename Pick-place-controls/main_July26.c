...
// CONSTANTS -------------------------------------------------------------------------
float kp = 14, kd = 10, initialAngle = 90, finalAngle = 45;

// VARIABLES -------------------------------------------------------------------------
float lastError = 0, currentAngle = 0, error, p, d, flag = 0, object = 0, output, qeiPosition = 0, qeiRotation = 0, qeiDegrees = 0, degrees = 0;
...
void Timer0IntHandler(void){
   ...
   // if object has been picked up use final Angle as reference
      ...
      error = finalAngle - degrees;
   }

     p = kp * error;
     d = kd * (error - lastError);
     output = p + d; // note: PWM doesn't really have an effect when output < 80
     lastError = error;

 // if error is negative, flag = 0, if positive, flag = 1
     if(output < 0){
         output = (-1.0*output);
         flag = 0; // go CCW
     }
 ...
 // Setting max input duty cycle, so it never hits 100%.
     if (output >= 500) output = 500;


     //else if (output < 80 && output > 40) output = 80;
     ...
     if(flag==0) //CCW
     {
         ...
         GPIOPinTypePWM(GPIO_PORTB_BASE, GPIO_PIN_7); //make pin 7 output for Q2/Q3 (CC)
...
void main(void) {
       ...
       while(1){
           ...
           qeiDegrees = qeiPosition*360.0/12606.0;
        }
}
